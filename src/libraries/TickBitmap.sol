...
library TickBitmap {
        ...
        if (lte) {
            ...
            next = initialized
                ? (compressed +
                    1 +
                    int24(
                        BitMath.leastSignificantBit(masked) - uint24(bitPos)
                    )) * tickSpacing
    ...
    function nextInitializedTick(
        ...
        nextTick = lte ? tick : tick;
        while (!initialized) {
            (nextTick, initialized) = nextInitializedTickWithinOneWord(
                self,
                nextTick, // @todo debug that issue when going backward in the ticks,
                lte
            );
            nextTick = lte && !initialized ? nextTick - 1 : nextTick;
